...
from database_models import EPF_NPC, EPF_Item
...
async def EPF_import_item(file: str, async_session):
    try:
        with open(f"{file}", encoding='utf8') as f:
            # logging.info(f'{file}')
            data = json.load(f)
            if "type" in data.keys() and data['type'] == 'weapon':
                ...
                try:
                    ...
                    if os.environ['Overwrite'] == "True":
                        async with async_session() as session:
                            item_result = await session.execute(select(EPF_Item).where(EPF_Item.name == data['name']))
